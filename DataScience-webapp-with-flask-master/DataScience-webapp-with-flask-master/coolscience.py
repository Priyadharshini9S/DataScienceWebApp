...
@app.route('/datasets/<dataset>/model_process/', methods=['POST'])
def model_process(dataset=dataset):
    alg = request.form.get('model')
    ...
    if variables != [] and '' not in variables: 
     df = df.dropna[list(set(variables + [res]))] 
     X = df(str(res), axis=1)
     y = df[str(res)]
     ...
     if score == 'Classification':
         ...
         fig = plotfun.plot_ROC(X, y, mod, cv=str(kfold))
         ...
         fig = plotfun.plot_predVSreal(X, y, mod, cv=str(kfold))
    
         scores = cross_validate(mod, X, y, cv=str(kfold), scoring=scoring)
         for s in scores:
         ...
         return render_template('scores.html', scores = scores, dataset = dataset, alg=alg,
                           ...
                           predictors = pred, response = int(fig))
...
def preprocessed_dataset(dataset):
    ...
    try:
        ...
        #Reduced Dataset
        df = pd.DataFrame(kbest.fit_transform(X),columns=best_features)
        df.insert(0, str(response), y)
        
        filename += str(numFeatures) + '_' + 'NA' + str(dropna) + '_Same' + str(dropsame) + '.csv'
...
def graph_process(dataset = dataset):
    ...
    boxcat = request.form.get('boxcat')
    boxconcat = request.form.get('boxconcat')
    ...
    #if boxcat != '' and boxcat != '':
        #figs['Box Plot'] = str(plotfun.plot_boxplot(ds, boxcat, boxconcat), 'utf-8')
    if figs == {}: return redirect('/datasets/'+ str(dataset) +'/graphs')
    # print(figs)
...
def predict_process(dataset = dataset):
    ...
    if score == 'Classification':
        ...
        pred_proba = model.predict(Xpred)
